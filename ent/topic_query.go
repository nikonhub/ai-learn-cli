...
func (tq *TopicQuery) QueryItems() *ItemQuery {
...
func (tq *TopicQuery) WithItems(opts ...func(*ItemQuery)) *TopicQuery {
	query := (&ItemClient{config: tq.config}).Query()
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withItems; query != nil {
		if err := tq.loadItems(ctx, query, nodes,
			func(n *Topic) { n.Edges.Items = []*Item{} },
			func(n *Topic, e *Item) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Item)) error {
	...
	query.Where(predicate.Item(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(topic.ItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.topic_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "topic_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "topic_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
